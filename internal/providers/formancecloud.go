...
type FormanceCloudProvider struct {
	...
	SDKFactory membership.SDKFactory
}
...
func (p *FormanceCloudProvider) Configure(ctx context.Context, req provider.ConfigureRequest, resp *provider.ConfigureResponse) {
	...
	var data internal.FormanceCloudProviderModel
	...
	cli, tp := p.SDKFactory(p.Version, &internal.FormanceCloudProviderModel{
...
func New(logger logging.Logger, version, endpoint, clientId, clientSecret string, sdkFactory membership.SDKFactory) func() provider.Provider {
